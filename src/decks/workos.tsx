  ...
  return (
    <SlideFrame>
      ...
      <div className="flex-1 flex flex-col justify-center relative">
        ...
        <p
          className="text-[40px] leading-snug max-w-[1500px] mb-20"
          style={{ color: "#E2E8F0" }}
        >
        ...
        <div className="flex items-center gap-12 mb-20">
          {["Cursor", "Claude", "Shell scripts"].map((t, i) => (
            <span key={t} className="flex items-center gap-12">
              <span
                className="px-6 py-2 rounded-full font-mono text-[28px]"
                style={{
                  background: "#1E293B",
  ...
  return (
    <SlideFrame>
      ...
      <div className="grid grid-cols-2 gap-12 flex-1 min-h-0">
        <div
          className="rounded-2xl p-12 font-mono text-[28px] leading-relaxed whitespace-pre-wrap"
          style={{ background: CARD_BG, color: MUTED }}
        >
          <div className="text-[26px] mb-10" style={{ color: BLUE }}>
...
need to decide before sprint. TBD`}
        ...
        <div className="flex flex-col justify-center gap-12">
          {pains.map((p) => (
            <div
              key={p}
              className="flex items-start gap-7 rounded-xl p-10"
              style={{ background: CARD_BG }}
            >
              <span className="text-[42px] leading-none" style={{ color: AMBER }}>
                ⚠
              </span>
              <span className="text-[32px] leading-snug">{p}</span>
            </div>
          ))}
        </div>
      </div>
      <div
        className="mt-10 rounded-xl px-10 py-8 border-l-4"
    ...
    <SlideFrame>
      ...
      <div className="flex items-stretch gap-12 mb-10">
        <div className="flex flex-col gap-3 w-[360px]">
          <div
            className="font-mono text-[22px] uppercase tracking-[0.2em] mb-1"
            style={{ color: MUTED }}
          >
            Sources
          </div>
          {sources.map((s) => (
            ...
            >
              ...
              <div className="px-5 py-4">
                <div
                  className="font-mono text-[22px] mb-1"
                  style={{ color: s.color }}
                >
                  {s.folder}
                </div>
                <div className="text-[24px]" style={{ color: "#E2E8F0" }}>
                  {s.desc}
                </div>
              </div>
            </div>
          ...
          {pipeline.map((b, i) => (
            <div key={b.folder} className="flex items-stretch gap-3 flex-1">
              ...
              >
                ...
                <div className="p-7">
                  <div
                    className="font-mono text-[24px] mb-2"
                    style={{ color: b.color }}
                  >
                    {b.folder}
                  </div>
              ...
              {i < pipeline.length - 1 && (
                <div className="flex items-center text-[34px]" style={{ color: MUTED }}>
                  →
                </div>
              )}
            </div>
      ...
      <div
        className="flex items-center gap-12 mb-10 rounded-xl px-6 py-4"
        style={{ background: `${EMERALD}10`, border: `1px dashed ${EMERALD}55` }}
      >
        <span className="text-[30px]" style={{ color: EMERALD }}>↺</span>
        <span className="text-[26px]" style={{ color: "#E2E8F0" }}>
          Shipped specs generate new feedback and data — feeding the next loop.
          Work isn't linear; it's a cycle.
        </span>
      </div>
      <div className="grid grid-cols-2 gap-12 flex-1 min-h-0">
        <div className="rounded-2xl p-10" style={{ background: CARD_BG }}>
          <div
            className="font-mono text-[26px] uppercase tracking-[0.18em] mb-10"
            style={{ color: MUTED }}
          >
            Before — Raw
          </div>
          <p className="text-[30px] font-mono leading-snug" style={{ color: MUTED }}>
    ...
    <SlideFrame>
      ...
      <div className="grid grid-cols-3 gap-12 flex-1 min-h-0">
        {tracks.map((t) => (
          <div
            key={t.title}
            className="rounded-2xl p-10 flex flex-col border-2"
            style={{
              background: CARD_BG,
          ...
          >
            <span
              className="self-start px-3 py-1 rounded-md font-mono text-[22px] mb-7"
              style={{ background: `${t.pillColor}22`, color: t.pillColor }}
            >
              {t.pill}
            </span>
            <h2 className="text-[40px] font-bold mb-3">{t.title}</h2>
            <div className="font-mono text-[26px] mb-10" style={{ color: MUTED }}>
              {t.folder}
            </div>
            <p className="text-[28px] leading-snug flex-1 mb-10">{t.body}</p>
            <div
              className="text-[24px] font-mono pt-5 border-t"
      ...
      <p
        className="mt-8 text-[30px] text-center italic"
        style={{ color: "#E2E8F0" }}
      >
...
function S5() {
  ...
  return (
    <SlideFrame>
      <h1 className="text-[52px] font-extrabold leading-tight tracking-tight mb-10">
      ...
      <div className="grid grid-cols-2 gap-12 flex-1 min-h-0">
        {items.map((it) => (
          ...
          >
            ...
            <p className="text-[26px] leading-snug" style={{ color: "#E2E8F0" }}>
              {it.body}
            </p>
          </div>
        ))}
      ...
      <div
        className="mt-8 rounded-xl px-8 py-6 border-l-4"
        style={{ background: `${EMERALD}14`, borderColor: EMERALD }}
      >
...
function S6() {
  ...
  return (
    <SlideFrame>
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] gap-12 items-center flex-1 min-h-0">
        <div className="flex flex-col gap-12">
          <div
            className="font-mono text-[24px] uppercase tracking-[0.2em] mb-2"
            style={{ color: EMERALD }}
          >
            WorkOS
          </div>
          {workos.map((s) => (
            <div
              key={s}
              className="rounded-xl p-10 flex items-center gap-12"
              style={{ background: CARD_BG }}
            >
              <span className="text-[34px]" style={{ color: EMERALD }}>
                →
              </span>
        ...
        <div
          className="text-[28px] font-bold text-center px-8 py-6 rounded-xl max-w-[220px]"
        ...
        <div className="flex flex-col gap-12">
          ...
          {firstDue.map((s) => (
            <div
              key={s}
              className="rounded-xl p-10 flex items-center gap-12 border-2"
              style={{ background: CARD_BG, borderColor: BLUE }}
            >
              <span className="text-[34px]" style={{ color: BLUE }}>
                →
              </span>
              <span className="text-[30px]">{s}</span>
            </div>
...
function S7() {
  ...
  return (
    <SlideFrame>
        ...
        {rows.map((r) => (
          ...
          >
            ...
            <div className="flex-1">
              <div className="text-[36px] font-bold mb-2">{r.t}</div>
      ...
      <p
        className="mt-8 text-[32px] font-semibold italic"
        style={{ color: EMERALD }}
      >
